<?php

declare(strict_types=1);

namespace App\Support;

final class AdvertFormValidator
{
    /**
     * @return list<string>
     */
    public function validateCreation(array $payload): array
    {
        $errors = $this->validateSharedFields($payload);

        $password = trim((string) ($payload['psw'] ?? ''));
        $passwordConfirmation = trim((string) ($payload['confirm-psw'] ?? ''));

        if ($password === '' || $passwordConfirmation === '' || $password !== $passwordConfirmation) {
            $errors[] = 'Les mots de passes ne sont pas identiques';
        }

        return $errors;
    }

    /**
     * @return list<string>
     */
    public function validateEdition(array $payload): array
    {
        return $this->validateSharedFields($payload);
    }

    /**
     * @return list<string>
     */
    private function validateSharedFields(array $payload): array
    {
        $name = trim((string) ($payload['nom'] ?? ''));
        $email = trim((string) ($payload['email'] ?? ''));
        $phone = trim((string) ($payload['phone'] ?? ''));
        $city = trim((string) ($payload['ville'] ?? ''));
        $department = trim((string) ($payload['departement'] ?? ''));
        $category = trim((string) ($payload['categorie'] ?? ''));
        $title = trim((string) ($payload['title'] ?? ''));
        $description = trim((string) ($payload['description'] ?? ''));
        $price = trim((string) ($payload['price'] ?? ''));

        $errors = [];

        if ($name === '') {
            $errors[] = 'Veuillez entrer votre nom';
        }

        if (!$this->isEmail($email)) {
            $errors[] = 'Veuillez entrer une adresse mail correcte';
        }

        if ($phone === '' && !is_numeric($phone)) {
            $errors[] = 'Veuillez entrer votre numéro de téléphone';
        }

        if ($city === '') {
            $errors[] = 'Veuillez entrer votre ville';
        }

        if (!is_numeric($department)) {
            $errors[] = 'Veuillez choisir un département';
        }

        if (!is_numeric($category)) {
            $errors[] = 'Veuillez choisir une catégorie';
        }

        if ($title === '') {
            $errors[] = 'Veuillez entrer un titre';
        }

        if ($description === '') {
            $errors[] = 'Veuillez entrer une description';
        }

        if ($price === '' || !is_numeric($price)) {
            $errors[] = 'Veuillez entrer un prix';
        }

        return $errors;
    }

    private function isEmail(string $email): bool
    {
        return preg_match(
            "/^[-_.[:alnum:]]+@((([[:alnum:]]|[[:alnum:]][[:alnum:]-]*[[:alnum:]])\.)+(ad|ae|aero|af|ag|ai|al|am|an|ao|aq|ar|arpa|as|at|au|aw|az|ba|bb|bd|be|bf|bg|bh|bi|biz|bj|bm|bn|bo|br|bs|bt|bv|bw|by|bz|ca|cc|cd|cf|cg|ch|ci|ck|cl|cm|cn|co|com|coop|cr|cs|cu|cv|cx|cy|cz|de|dj|dk|dm|do|dz|ec|edu|ee|eg|eh|er|es|et|eu|fi|fj|fk|fm|fo|fr|ga|gb|gd|ge|gf|gh|gi|gl|gm|gn|gov|gp|gq|gr|gs|gt|gu|gw|gy|hk|hm|hn|hr|ht|hu|id|ie|il|in|info|int|io|iq|ir|is|it|jm|jo|jp|ke|kg|kh|ki|km|kn|kp|kr|kw|ky|kz|la|lb|lc|li|lk|lr|ls|lt|lu|lv|ly|ma|mc|md|mg|mh|mil|mk|ml|mm|mn|mo|mp|mq|mr|ms|mt|mu|museum|mv|mw|mx|my|mz|na|name|nc|ne|net|nf|ng|ni|nl|no|np|nr|nt|nu|nz|om|org|pa|pe|pf|pg|ph|pk|pl|pm|pn|pr|pro|ps|pt|pw|py|qa|re|ro|ru|rw|sa|sb|sc|sd|se|sg|sh|si|sj|sk|sl|sm|sn|so|sr|st|su|sv|sy|sz|tc|td|tf|tg|th|tj|tk|tm|tn|to|tp|tr|tt|tv|tw|tz|ua|ug|uk|um|us|uy|uz|va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|yu|za|zm|zw)$|(([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5])\.){3}([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5]))$/i",
            $email
        ) === 1;
    }
}
